...
def aff_pol_2(T,t,x,y,C,text_bold=False,text_italic=False,police=False) :
    """
    Affiche une chaîne de caractère T en police Verdana à la taille t.
    ...
    Les arguments text_bold (gras) et text_italic (italique) sont optionnels.
    """
    if PYGAME_SDL_FONT in pygame.font.get_fonts() :
        font = pygame.font.SysFont(PYGAME_SDL_FONT,t,bold=text_bold,italic=text_italic)
    else:
        font = pygame.font.Font(PYGAME_SDL_FONT, t,bold=text_bold,italic=text_italic)
    text = font.render(T,1,C)
    PYGAME_SDL_WINDOW.blit(text,(x,y))
...
def draw_rectangle_2(x,y,l,h,C) :
    """
    Trace un rectangle de couleur C, de centre P, de largeur l et de hauteur h.
    """
    pygame.draw.rect(PYGAME_SDL_WINDOW,C,(int(x-l/2),int(y-h/2),int(l),int(h)),1)
...
def draw_fill_rectangle_2(x,y,l,h,C) :
    """
    Trace un rectangle plein de couleur C, de centre x,y, de largeur l et
    de hauteur h.
    """
    pygame.draw.rect(PYGAME_SDL_WINDOW,C,(int(x-l/2),int(y-h/2),int(l),int(h)),0)
